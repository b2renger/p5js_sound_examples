...
function setup() {
  ...
  canvas.canvas.style.display = 'block'; // Removes scroll bars issues
  ...
  startBtn.addEventListener('click', () => {
    ...
    // Get the initial number from the input
    let numStr = document.getElementById('num-boids').value;
    let numBoids = parseInt(numStr, 10);
    if (isNaN(numBoids) || numBoids < 1) numBoids = 50;
    ...
    setTimeout(() => {
      ...
      // Initialize GUI only after starting
...
var initGui = function () {
  ...
  f2.add(sp, 'separate', 0.0, 5.0).step(0.1);
  f2.add(sp, 'align', 0.0, 5.0).step(0.1);
  f2.add(sp, 'cohesion', 0.0, 5.0).step(0.1);
  f2.add(sp, 'add_boid');
